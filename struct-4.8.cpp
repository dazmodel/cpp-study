...
void printStructuresSet(student *students, int n) {
  ...
  for ( int i = 0; i < n; i++ ) {
    ...
    cout << "     Sportsmen?  : " << students[i].doingSport << endl;
...
// Поиск спортсменов
vector<student> findSportsmen(student *students, int nStuents) {
  vector<student> result;
  for ( int i = 0; i < nStuents; i++ ) {
    if ( students[i].doingSport ) {
...
void printGroups(vector<student> s, int nDisc) {
  cout << "Spisok studentov-sportsmenov, sdav6ih vse na 4 i 5: " << endl;
  cout << endl;
  for ( int i = 0; i < s.size(); i++ ) {
    int fourFiveCount = 0;
    for ( int j = 0; j < nDisc; j++ ) {
      if ( s[i].disciplines[j].mark >= 4 ) {
        fourFiveCount++;
      }
    }

    if ( fourFiveCount == nDisc ) {
...
int main() {
  ...
  // Ищем спортсменов
  vector<student> sportsmen = findSportsmen(students, numberOfStudents);

  // Печатаем спортсменов, сдавших сессию на 4 и 5
  printGroups(sportsmen, numberOfDisciplines);
